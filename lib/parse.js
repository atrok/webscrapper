...
var requestHandler = function (args) {
    ...
    var parserComponentLinks = async function (html, component) {
        ...
        var prepareObject = function (family, solution_name, embed) {
            ...
            logger.info("Component "+el.component+" parsed succesfully");
            logger.debug("Constructed object: "+JSON.stringify(el, 2));
        ...
        await new Promise((resolve, reject) => {
            ...
            families.each((i, val) => {
                ...
                logger.info("Start resolving solutions for family "+family);
                ...
                if (parsedFamilies.arr.indexOf(family) != -1) {
                    ...
                    h4.each((ii, h) => {
                        ...
                        logger.info("Found solution: "+solution_name+", resolving components");
                        ...
                        if (embed.length > 0) { //found embed link
                           ...
                           if (_$(h).text() == 'Composer') {
                            console.log('Bingo');
                        }
                        */
                        var h4tag=_$(h)[0].name;
                            ...
                            _$(h).nextAll('ul').each((o, ul) => {
                                ...
                                // 12/2/2019
                                // fix of the issue with parsing components
                                // if it's listed in format <h4></h4><ul></ul> without being wrapped into <div>

                                if(h4tag==_$(ul).prev()[0].name && solution_name!==_$(ul).prev().text()){
                                    logger.info("Found component doesn't belong to <"+h4tag+">"+solution_name+"</"+h4tag+"> but <"+_$(ul).prev()[0].name+">"+_$(ul).prev().text()+"</"+_$(ul).prev()[0].name+">, aborting!!");
                                }else{
                                let f = family;
                                let s = solution_name;
                ...
                if (!cache.isCached(options.url)) {
                    ...
                    request(options, function (error, response, html) {
                        ...
                        if (error) {
                            ...
                            // 11/25/2019
                            // process would stop if response code is other than 200
                            // adding new logic to cover this
                            }else{
                                reject("Can't process "+options.url + ", HTTP Status code: " + response.statusCode)
                            }
                        }
